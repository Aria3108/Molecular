...
mat distMatADN( vector<string> seq){
    int n = seq.size();
...
float distAP(string a, string b) {
...
mat distMatAP( vector<string> seq){
    int n = seq.size();
    mat dist(n, vector<double>(n,0.0));
...
int main(){
    ...
    mat distAP = distMatAP(apell);  
    printMat(distAP);
    vector<string> etiquetasAP = { "A1", "A2", "A3", "A4", "A5" };
    Node* rootAP = UPGMA(etiquetasAP, distAP);
    cout << "\n Arbol de apellidos en notacion Newick:\n";
    printNewick(rootAP);
    cout << ";" << endl;

    exportToDOT(rootAP, "arbolAP.dot");
    cout << "\nArchivo DOT exportado: arbolAP.dot" << endl;


    //NEIGHBOR JOIN
    cout << endl << "-----------------------NJ ADN----------------------" << endl;
    ...
    cout << endl << "-----------------------NJ APELLIDOS----------------------" << endl;

    
    Node* rootNJA = NeighborJoining(etiquetasAP, distAP);
    cout << "\n ArbolNJ en notacion Newick:\n";
    printNewick(rootNJA);
    cout << ";" << endl;
    exportToDOT(rootNJA, "arbolNJAP.dot");
    cout << "\nArchivo DOT exportado: arbolNJAP.dot" << endl;
    return 0;
}